    ...
    config = {
      ...
      text =
        ...
        (
          ...
          ''
          + (let
            inherit (config.security.pam) p11;
          in
            optionalString cfg.p11Auth ''
              auth ${p11.control} ${pkgs.pam_p11}/lib/security/pam_p11.so ${pkgs.opensc}/lib/opensc-pkcs11.so
            '')
          + (let
            inherit (config.security.pam) u2f;
          in
          ...
          ''
          + (let
            inherit (config.security.pam) ussh;
          in
            ...
            '')
          + (let
            inherit (config.security.pam) oath;
          in
...
in {
  ...
  config = {
    assertions = [
      {
        assertion = null config.users.motd || null config.users.motdFile;
        message = ''
